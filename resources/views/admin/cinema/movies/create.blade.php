...
@section('content')
<div class="mb-8">
    <h2 class="text-2xl font-bold text-gray-800">{{ isset($movie) ? 'Chỉnh Sửa Phim' : 'Thêm Phim Mới' }}</h2>
    <p class="text-sm text-gray-500 mt-1">{{ isset($movie) ? 'Cập nhật thông tin phim' : 'Thêm bộ phim mới vào hệ thống' }}</p>
</div>

<div class="bg-white rounded-lg shadow-sm border border-gray-100 max-w-2xl">
    <form action="{{ isset($movie) ? route('admin.movies.update', $movie->id) : route('admin.movies.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
        @csrf
        @if(isset($movie))
            @method('PUT')
        @endif
        ...
        <div>
            ...
            <input type="text" id="title" name="title" value="{{ $movie->title ?? old('title') }}" required
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4"
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                      placeholder="Nhập mô tả phim">{{ $movie->description ?? old('description') }}</textarea>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label for="duration" class="block text-sm font-bold text-gray-700 mb-2">Thời Lượng (phút) *</label>
                <input type="number" id="duration" name="duration" value="{{ $movie->duration ?? old('duration') }}" required min="1"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
            ...
            <div>
                ...
                <input type="date" id="release_date" name="release_date" value="{{ $movie->release_date ?? old('release_date') }}" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" id="genre" name="genre" value="{{ $movie->genre ?? old('genre') }}"
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
            ...
            <button type="submit" class="px-6 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition font-semibold">
                {{ isset($movie) ? 'Cập Nhật' : 'Thêm Phim' }}
            </button>
        </div>
    </form>
</div>
